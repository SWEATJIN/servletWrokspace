...
public class MariaDao extends CommonModule {
	...
	public MariaDao() {
		super(context, context.getInitParameter("maria_driver"), context.getInitParameter("maria_url"),
				context.getInitParameter("maria_userName"), context.getInitParameter("maria_psw"));
	}

	@Override
	public ArrayList<NoticeView> getNotices(int pageNum, String searchField, String searchWord, boolean pub)
			throws SQLException {
		String selectSql = String.format(
				"SELECT * FROM notice_view " + "WHERE %s LIKE ? %s " + "ORDER BY REGDATE DESC " + "LIMIT %d, %d;",
	...
	public int getNoticeCnt(String searchField, String searchWord, boolean pub) throws SQLException {
		String selectSql = String.format("SELECT COUNT(ID) CNT FROM NOTICE WHERE %s LIKE ? %s", searchField,
				pub ? "" : "AND pub = 1");
	...
	public Notice getPrevNotice(int id, String searchField, String searchWord, boolean pub) throws SQLException {
		String selectSql = String.format("SELECT * FROM NOTICE " + "WHERE %s %s LIKE ? "
				+ "AND REGDATE < (SELECT REGDATE FROM NOTICE WHERE ID = ?)" + "ORDER BY REGDATE DESC " + "LIMIT 1",
	...
	public Notice getNextNotice(int id, String searchField, String searchWord, boolean pub) throws SQLException {
		String selectSql = String.format(
				"SELECT * FROM NOTICE " + "WHERE %s %s  LIKE ? "
						+ "AND REGDATE > (SELECT REGDATE FROM NOTICE WHERE ID = ?)" + "LIMIT 1",
				pub ? "" : "pub = 1 AND", searchField);
