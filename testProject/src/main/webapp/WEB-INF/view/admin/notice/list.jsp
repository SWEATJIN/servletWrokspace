...
<body>
   <div id="body">
      <main class="main">
         ...
         <!--
            EL 내에서의 연산은 자바 기준이 아닌 js 와 같은 형태로 연산 결과 산출됨에 주의.
            따라서 아래와 같은 연산식으로는 정상적 결과 산출 불가.
            
            ※ 기존 pageNationLastNum 에서 wholePage 로 변수명 변경.
              여기서 wholePage 전체 레코드수가 아닌, 레코드 pagingSizeValue 묶음에 따른 전체 페이지 네이션 갯수를 의미함에 주의. 
         -->
<%--          <c:set var="wholePage" value="${noticeCnt/pagingSizeValue + (noticeCnt % pagingSizeValue > 0 ? 1 : 0 )}"/> --%>
         
         <!-- 아래 코드는 정상 처리 -->
         <c:set var="wholePage" value="${ Math.floor( noticeCnt / pagingSizeValue ) + ( noticeCnt % pagingSizeValue > 0  ? 1 : 0 ) }"/>
<%--          <c:set var="wholePage" value="${ Math.ceil(noticeCnt / pagingSizeValue) }"/> --%>
         ...
         <div class="pageNationPart">
            <div>
               ...
               <div>
                  <span>${ pageNum}</span> / <fmt:formatNumber value="${wholePage}" maxFractionDigits="0"/> pages
<%--                   <span>${ pageNum }</span> / <fmt:formatNumber value="${wholePage}" pattern="####"/> pages --%>
